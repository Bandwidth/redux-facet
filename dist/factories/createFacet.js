 ...
 */
export default (selectors => (facetName, baseMapStateToProps, baseMapDispatchToProps, baseMergeProps, options = {
  ...
  const mapStateToProps = (state, ownProps) => _extends({}, baseMapStateToProps(selectors.selectFacetState(facetName)(state), ownProps, state), {
  ...
  const mapDispatchToPropsInjectingFacetName = (dispatch, ownProps) => {
    ...
    return _extends({}, baseMapDispatchToProps ? baseMapDispatchToProps(facetDispatch, ownProps, dispatch) : {}, {
      facetDispatch
    });
  };

  return options.connect(mapStateToProps, mapDispatchToPropsInjectingFacetName, baseMergeProps,
  // pass through connect options from HOC options
  _.pick(options, ['pure', 'areStatesEqual', 'areOwnPropsEqual', 'areStatePropsEqual', 'areMergedPropsEqual', 'storeKey']));
});